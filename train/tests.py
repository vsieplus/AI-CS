...
class TestStepchart(unittest.TestCase):
	...
	def test_special_tokens(self):
		vs = SELECTION_VOCAB_SIZES['pump-double']
		special = {vs: 'XXXXXXXXXX', vs + 1: '..MMXMM..'}
		...
		self.assertEqual(new_targets[0].item(), vs + 2)
		self.assertEqual(new_targets[1].item(), vs + 3)
		self.assertEqual(new_tokens, 2)

		self.assertEqual(special[vs + 2], 'XXWWXXWWXX')
		self.assertEqual(special[vs + 3], 'XX..XX..XX')
